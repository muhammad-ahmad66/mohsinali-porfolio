...
export interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  ...
  fullWidth?: boolean;
  loading?: boolean;
  showArrowOnHover?: boolean; // New prop for arrow animation
}
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    const variantStyles = {
      primary:
        'bg-primary-600 text-white shadow-md shadow-primary-600/20 hover:bg-primary-500 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-primary-500/25 focus-visible:ring-primary-500 dark:bg-primary-600 dark:hover:bg-primary-500',
      secondary:
        'bg-secondary-600 text-white shadow-md shadow-secondary-600/20 hover:bg-secondary-500 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-secondary-500/25 focus-visible:ring-secondary-500 dark:bg-secondary-600 dark:hover:bg-secondary-500',
      outline:
        'border-2 border-primary-600 text-primary-600 bg-transparent hover:bg-primary-50 hover:border-primary-700 focus-visible:ring-primary-500 dark:border-primary-500 dark:text-primary-400 dark:hover:bg-primary-950/50 dark:hover:border-primary-400',
      ghost: 'text-foreground hover:bg-accent hover:text-accent-foreground',
      destructive:
        'bg-red-600 text-white shadow-md shadow-red-600/20 hover:bg-red-500 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-red-500/25 focus-visible:ring-red-500',
      link: 'text-primary-600 underline-offset-4 hover:underline dark:text-primary-400',
    };
    ...
    // Determine if we should show the arrow (for primary CTAs)
    const shouldShowArrow =
      showArrowOnHover || (variant === 'primary' && !Icon);
      ...
      >
        {asChild ? (
          ...
          <>
            ...
            {/* Arrow that appears on hover for primary CTAs */}
            {!loading && shouldShowArrow && !Icon && (
              <ArrowRight
                className={cn(
                  iconSizes[size],
                  'translate-x-0 opacity-0 transition-all duration-200 group-hover:translate-x-1 group-hover:opacity-100'
                )}
              />
            )}
          </>
